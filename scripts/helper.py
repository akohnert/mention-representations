...
def convert_coref_ua_to_json(UA_PATH, JSON_PATH, MODEL="coref-hoi", SEGMENT_SIZE=384, TOKENIZER_NAME="bert-base-cased"):
    if MODEL == "coref-hoi":
...
'''          
def convert_bridg_ua_to_convert_coref_json_to_ua_dali_bridging(UA_PATH, JSON_PATH):
...
def convert_coref_json_to_ua(JSON_PATH, UA_PATH, MODEL="coref-hoi"):	
    ...
    with open(JSON_PATH, "r") as f:
        ...
        # some json objects are not delimited by line break!
        d = "}"
        text = f.readlines()
        if len(text) == 1:
            text = text[0]
