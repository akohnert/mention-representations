...
class Runner:
    ...
    def initialize_model(self, saved_suffix=None):
        ...
        #self.load_model_checkpoint(model, 'ontonotes_arrau_speaker')   # load pre-trained model
        #model.add_special_speaker_tokens()             # if special speaker tokens added in training, add for inference too!
        if saved_suffix:
    ...
    def train(self, model):
        ...
        for epo in range(epochs):
            ...
            for doc_key, example in examples_train:
                ...
                predictions, loss = model(*example_gpu)
                _, _, candidate_mention_scores, span_starts, span_ends, antecedent_idx, antecedent_scores = predictions
                if not(isinstance(span_starts, list)):
                ...
                if len(loss_during_accum) % grad_accum == 0:
                    ...
                    # Evaluate
                    if len(loss_history) % conf['eval_frequency'] == 0:
    ...
    def evaluate(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None):
        ...
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            ...
            tensor_example = tensor_example[:8]  # Strip out gold
            example_gpu = [d.to(self.device) for d in tensor_example]
            with torch.no_grad():
                candidate_starts, candidate_ends, candidate_mention_scores, span_starts, span_ends, antecedent_idx, antecedent_scores = model.get_predictions_and_loss(*example_gpu)
            span_starts, span_ends = span_starts.tolist(), span_ends.tolist()
            antecedent_idx, antecedent_scores = antecedent_idx.tolist(), antecedent_scores.tolist()
            predicted_clusters = model.update_evaluator(candidate_starts, candidate_ends, candidate_mention_scores, span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, evaluator)
    ...
    def predict(self, model, tensor_examples, use_gold_mentions):
        ...
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            if use_gold_mentions:
                example_gpu = [d.to(self.device) for d in tensor_example]
                with torch.no_grad():
                    candidate_starts, candidate_ends, candidate_mention_scores, span_starts, span_ends, antecedent_idx, antecedent_scores = model.get_predictions_and_loss(*example_gpu, gold_prediction_mode=True)
            else:
                tensor_example = tensor_example[:9]
                example_gpu = [d.to(self.device) for d in tensor_example]
                with torch.no_grad():
                    candidate_starts, candidate_ends, candidate_mention_scores, span_starts, span_ends, antecedent_idx, antecedent_scores = model(*example_gpu)
            span_starts, span_ends = span_starts.tolist(), span_ends.tolist()
            antecedent_idx, antecedent_scores = antecedent_idx.tolist(), antecedent_scores.tolist()
            clusters, mention_to_cluster_id, antecedents = model.get_predicted_clusters(candidate_starts, candidate_ends, span_starts, span_ends, antecedent_idx, antecedent_scores, candidate_mention_scores)
